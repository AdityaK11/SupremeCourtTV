...
data_dir = f'/home/akathpalia/data/{case_name}'
video_dir = f'{data_dir}/vids/'
output_dir = f'{data_dir}/outputs1/'
...
def merge_videos_with_transition(video_files, data, transition_duration=1):
    ...
    for file in video_files:
        ...
        try:
            ...
            # Set target resolution based on the first clip
            ...
            if clip.audio is None:
                print(f"Warning: No audio found in clip {file}. Creating silent audio.")
                clip = clip.set_audio(AudioClip(lambda t: [0], duration=clip.duration))
            # Write individual clip to file
            temp_output = os.path.join(video_dir, f'temp_{file}')
            clip.write_videofile(temp_output, codec='libx264', audio_codec='aac')            
            # Read the clip back to ensure audio is preserved
...
# def merge_videos(output_filename='output.mp4'):
#     clips = []
#     n = 0

#     while True:
#         # Break the loop if we've checked a large number of files and found none
#         if n > 20:
#             print("No valid video files found.")
#             break
#         video_filename = f"{video_dir}/{n}.mp4"
...
#             if clip.duration > 8:
#                 clip = add_text_to_video(clip)
#             clips.append(clip)
#             print(f"Added {video_filename}")
#         except Exception as e:
...
try:
    final_clip.write_videofile(f"{output_file}", codec="libx264", audio_codec="aac")
    print("Final video with overlays and transitions created successfully.")
except Exception as e:
    print(f"Error writing final video file: {str(e)}")
finally:
    final_clip.close()
